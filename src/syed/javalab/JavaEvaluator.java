...
public class JavaEvaluator extends CodeEvaluator {
    ...
    private String testScript(Test test) {
        ...
        switch (test) {
            ...
            case CASE_SENSITIVE -> {
                script.append("diff -y -w -i --ignore-all-space user.out data\\${i}.out > diff.out\n");
            }
//            case TYPO -> {
//
//            }
...
//        script.append("       diff -y -w --ignore-all-space user.out data\\${i}.out > diff.out\n");
        ...
        for (TestIO test : this.getTestIOs()) {
            for (Test method : test.getMethods()) {
                testScript(method);
            }
        }
