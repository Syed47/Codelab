...
public abstract class CodeEvaluator {
    ...
    public void setTest(TestIO test) {
        if (test.getInput() == null || test.getOutput() == null) {
            io.ERROR("Test input and/or output cannot be null");
            return;
        }
        this.testIO.add(test);
    }

    public void setTestData(String input, String output) {
        setTestData(input, output, Test.SINGLE_LINE);
    }

    public void setTestData(String input, String output, Test type) {
        if (input == null || output == null) {
            io.ERROR("Test input and/or output cannot be null");
            return;
        }
        this.testIO.add(new TestIO(input, output, type));
    }

    public void setTestData(File input, File output, Test type) {
        this.setTestData(
            io.readlines(input.getAbsolutePath()),
            io.readlines(output.getAbsolutePath()),
            Test.MULTI_LINE
        );
    }
